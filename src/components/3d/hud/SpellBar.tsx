...
function formatKeyLabel(code: string | undefined): string | null {
	...
	if (parts[0] === "Key" && parts[1]?.startsWith("Key"))
		return parts[1].slice(3);
...
function Slot({
	...
	return (
		<div
			className="relative w-12 h-12 rounded-lg bg-white/10 border border-white/15 shadow-inner overflow-hidden"
			title={tooltip ?? title}
		>
	...
	return (
		<div className="pointer-events-none fixed left-1/2 bottom-4 -translate-x-1/2 flex items-center gap-2">
			...
			<Slot
				...
				readyAt={readyAtDash}
				tooltip={`Dash (${keyE ?? "E"})`}
				icon={<ZapIcon className="text-amber-300" />}
			/>
