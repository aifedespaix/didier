...
export function SpellBar() {
  ...
  const Slot = ({ label, readyAt, title }: { label: string | null; readyAt?: number; title: string }) => (
    <div className="relative w-12 h-12 rounded-lg bg-white/10 border border-white/15 shadow-inner overflow-hidden">
  ...
  return (
    <div className="pointer-events-none fixed left-1/2 bottom-4 -translate-x-1/2 flex items-center gap-2">
      <Slot label={null} title="Q" />
      <Slot label={null} title="W" />
      <Slot label={keyE} title="Dash" readyAt={readyAtDash} />
      <Slot label={null} title="R" />
    </div>
  );
}
