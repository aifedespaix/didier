...
>(({ isHost, localId, getLocalPos, remotes, onNetSend }, ref) => {
	...
	const walls = useMemo(() => {
		...
		return [
			// North wall (z = +half.z)
			{
				center: { x: 0, y: half.y, z: half.z },
				half: { x: half.x, y: half.y, z: t / 2 },
			},
			// South wall (z = -half.z)
			{
				center: { x: 0, y: half.y, z: -half.z },
			...
			// East wall (x = +half.x)
			{
				center: { x: half.x, y: half.y, z: 0 },
				half: { x: t / 2, y: half.y, z: half.z },
			},
			// West wall (x = -half.x)
			{
				center: { x: -half.x, y: half.y, z: 0 },
	...
	function spawn(proj: SpawnProjectileInput) {
		const id =
			proj.id ??
			`proj_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`;
		const p: Projectile = {
			...proj,
			id,
	...
	useFrame((_s, dt) => {
		...
		bolts.current.forEach((ent, id) => {
			...
			if (pr.dist >= pr.range) {
				...
				onNetSend({ t: "proj-despawn", id, reason: "end" });
				return;
			}
			...
			if (isHost) {
				...
				for (const cfg of OBSTACLE_ITEMS) {
					...
					const cx = Math.max(
						center.x - half.x,
						Math.min(sx, center.x + half.x),
					);
					const cy = Math.max(
						center.y - half.y,
						Math.min(sy, center.y + half.y),
					);
					const cz = Math.max(
						center.z - half.z,
						Math.min(sz, center.z + half.z),
					);
					const dx = sx - cx,
						dy = sy - cy,
						dz = sz - cz;
					if (dx * dx + dy * dy + dz * dz <= hitRadius * hitRadius) {
						...
						onNetSend({ t: "ob-hp", id: cfg.id, hp: newHp });
				...
				for (const w of walls) {
					...
					const cx = Math.max(
						w.center.x - w.half.x,
						Math.min(sx, w.center.x + w.half.x),
					);
					const cy = Math.max(
						w.center.y - w.half.y,
						Math.min(sy, w.center.y + w.half.y),
					);
					const cz = Math.max(
						w.center.z - w.half.z,
						Math.min(sz, w.center.z + w.half.z),
					);
					const dx = sx - cx,
						dy = sy - cy,
						dz = sz - cz;
		...
		for (let i = 0; i < toRemove.length; i++) {
			const id = toRemove[i];
			const p = explodeAt[i];
			despawn(id, p ? "hit" : undefined, p ?? undefined);
		}

		// Update explosions lifetime
		const now = performance.now();
		explosions.current = explosions.current.filter(
			(e) => now - e.startMs < e.durationMs,
		);
